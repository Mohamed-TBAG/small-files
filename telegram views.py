...
class GI():
    def __init__(self):
        ...
        self.username = input("[?] Username (only Channel):")
        self.token = ""
        if self.username[0] == "@":
    ...
    def send_request(self):
        ...
        data = f"_token={self.token}&test_mode=on&number-days=1&slct-day=-&slct-moth=-&slct-year=-&link-channel={self.username}&speed-hour=1800&views=10000"
        r = self.r.post(url,headers=headers,data=data)
        if r.text.find('You already used demo')>=0:
            input("[!] Change Your channel username to skip block And use VPN")
            exit()
        else:
            print("[+] Done Send 10000 Views To Your Channel")
            input("")
            exit()
