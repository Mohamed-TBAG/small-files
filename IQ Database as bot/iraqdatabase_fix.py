import telebot,requests,pyodbc,os
from telebot import types
token = '5430139726:AAEF-_LTDD9x9_R-sSsGEzYqm89VRwphUeI'
bot = telebot.TeleBot (token)
def search_card(card, place):
  conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};'
                      r'DBQ=C:\Users\{}\Desktop\s\\{}.accdb;'.format(os.getlogin(),place))
  print("started")
  cursor = conn.cursor()
...
def search_person(first_name, father_name, grand_name, birth, place):
    conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};'
                      r'DBQ=C:\Users\HP\Desktop\s\\'+place+'.accdb;')
    cursor = conn.cursor()
...
def phone(message):
 ...
 if str(message.from_user.id)== '5543149965':
    ...
    huks.row_width = 1
    huka = types.InlineKeyboardButton(text="- Dev",url="t.me/huks3")
    huks.add(huka)
    bot.reply_to(message,'- صديقي انت لم يتم التفعيل لك يرجى مراسلة المطور \n- @Huks3 , @i_m_q',reply_markup=huks)
    imq = bot.forward_message(5543149965, message.chat.id, message.id)
    bot.reply_to(imq,f'- يحاول يبعبص ({message.from_user.id}) .')
@bot.callback_query_handler(func=lambda m:True)
def qu(call):
        u = open('pay.txt').read()
        if str(call.message.chat.id) in u or str(call.message.chat.id) == '5543149965':
         global place
         if call.data == 'ar':
          place = 'اربيل'
